...
const Mine = () => {
  ...
  if (!difficulty) {
    return <Greeter handledifficulty={handleChangeDifficultyAndRestart} />;
  }
  ...
  return (
    <div className='min-h-screen min-w-screen flex flex-col overflow-scroll lg-tablets:items-center lg-tablets:justify-center bg-gradient-to-br from-[#008080] to-[#004040]"'>
      <div
        className={`lg-tablets:relative relative w-screen h-screen lg-tablets:h-max lg-tablets:w-[80%] flex flex-col justify-start items-center bg-[#c0c0c0] border-4 border-t-white border-l-white border-r-[#808080] border-b-[#808080] shadow-2xl`}
      >
        ...
        <Tabs defaultValue={difficulty} className="w-full bg-[#c0c0c0]">
          <TabsList className="w-full bg-[#c0c0c0] border-b-2 border-[#808080] rounded-none p-0">
            <TabsTrigger
              value="Easy"
              onClick={(e) => tabCickHandler(e.target.innerText)}
          ...
          <TabsContent key={`game-easy-${gameKey}`} value="Easy">
            <Game
              difficulty="Easy"
              onRestartRequest={handleChangeDifficultyAndRestart}
              levels={levels}
            />
          </TabsContent>
          <TabsContent key={`game-normal-${gameKey}`} value="Normal">
            <Game
              difficulty="Normal"
              onRestartRequest={handleChangeDifficultyAndRestart}
              levels={levels}
          ...
          <TabsContent key={`game-hard-${gameKey}`} value="Hard">
            <Game
              difficulty="Hard"
              onRestartRequest={handleChangeDifficultyAndRestart}
              levels={levels}
        ...
        <div className="w-full h-full flex items-end mt-2 mb-2 lg-tablets:mt-0 lg-tablets:mb-0">
          <button
            ...
            className={`bg-[#c0c0c0] border-2 border-t-white border-l-white border-r-[#808080] border-b-[#808080] px-3 py-1 font-mono text-sm text-black hover:bg-[#d0d0d0] active:border-t-[#808080] active:border-l-[#808080] active:border-r-white active:border-b-white`}
          >
            How to Play?
          </button>
        </div>
