...
const Game = ({ difficulty, levels, onRestartRequest }) => {
  ...
  const [bomb, setBomb] = useState(
    <GiLandMine
      className={`${difficulty === "Easy" && "size-6 -m-1 -ml-1.5"}
            ${difficulty === "Normal" && "size-4 mt-0.5 -ml-0.5"} ${
        difficulty === "Hard" && "-ml-0.5 mt-0.5"
      }`}
    />
  );
  ...
  useEffect(() => {
    if (difficulty) {
      ...
      setCorrectCells(
        levels[difficulty].rows * levels[difficulty].columns -
          levels[difficulty].bombs
      );
        ...
        setBomb(
          <GiMineExplosion
            className={`text-red-800 ${
              difficulty === "Easy" && "size-6 -m-1 -ml-1.5"
            }
            ${difficulty === "Normal" && "size-4 mt-0.5 -ml-0.5"} ${
              difficulty === "Hard" && "-ml-0.5 mt-0.5"
            }`}
  ...
  return (
    <div className="relative flex flex-col items-center h-full mx-auto w-full">
      <div
        className={`flex justify-between w-full px-2 py-1 bg-[#c0c0c0] border-2 border-t-white border-l-white border-r-[#808080] border-b-[#808080]`}
      >
        <div
          className={`${
            difficulty === "Hard"
              ? "flex gap-4 items-center px-2 w-35"
              : "flex flex-col"
          } justify-between bg-black border-2 border-t-[#808080] border-l-[#808080] border-r-white border-b-white p-2 font-mono text-red-500 font-bold`}
        >
          <p>Seconds </p>
          <p className="text-center">{timer}</p>
        </div>
        {gameOver ? (
          <Frown className="size-8 my-3 text-black fill-yellow-500" />
        ) : !gameWon ? (
          <Smile className="size-8 my-3 text-black fill-yellow-500" />
        ) : (
          <Laugh className="size-8 my-3 text-black fill-yellow-500" />
        )}
        <div
          className={`${
            difficulty === "Hard"
        ...
        >
          <p className="flex">
            Flags <FlagTriangleLeft className="text-yellow-400 fill-red-600" />
          </p>
          <p className="text-center">{flags}</p>
        </div>
      </div>
      <div
        className={`bg-[#969393] border-4 border-t-[#808080] border-l-[#808080] border-r-white border-b-white p-2`}
      >
        {mineGrid?.length > 0
          ? mineGrid.map((item, id) => {
              return (
                <div
                  className={`flex ${difficulty === "Normal" && "ml-6"}`}
                  key={id}
                >
                  {item.map((val, idx) => {
                    return (
                      <div
                        ...
                        className={`${
                          difficulty === "Easy" &&
                          "h-8 w-8 normal-phones:h-10 normal-phones:w-10 sm:h-10 sm:w-10 sm-laptops:h-10 lg-tablets:w-10 lg-tablets:h-10"
                        } 
                                 ${
                                   difficulty === "Normal" &&
                                   "h-4.5 w-4.5 normal-phones:h-6 normal-phones:w-6 lg-phone:h-6 lg-phones:w-6 sm:h-6 sm:w-6 "
                                 } 
                                 ${
                                   difficulty === "Hard" &&
                                   "h-3 w-3 normal-phones:h-3 normal-phones:w-3 lg-phones:h-4.5 lg-phones:w-4.5 sm:h-5 sm:w-5 "
                                 }
                      ...
                      >
                        {val.displayed ? (
                          <div
                            className={`${
                              difficulty === "Easy" &&
                              "ml-2 mt-1 normal-phones:p-2.5 normal-phones:ml-1"
                            } ${
                              difficulty === "Normal" &&
                              "ml-1 normal-phones:ml-2 normal-phones:mt-1 text-sm"
                            } ${
                              difficulty === "Hard" &&
                              "ml-0.5 -mt-0.5 normal-phones:ml-0.5 normal-phones:-mt-0.5 lg-phones:ml-1 lg-phones:mt-0 sm:ml-1.5 sm:mt-0.5 text-xs"
                            }`}
                          >
                            {val.item === "bomb" ? (
                          ...
                          <div className="flex flex-col h-full w-full">
                            <Image
                              src="/Minesweeper_unopened_square.svg"
                              alt="square"
                              priority
                              width={40}
                              height={30}
                            ...
                            {val.flagged && (
                              <FlagTriangleLeft
                                className={`text-black rotate-[-20deg] fill-red-600 absolute
                                      ${
                                        difficulty === "Easy" &&
                                        "size-6 mt-2 ml-1.5"
                                      }
                                      ...
                                      ${
                                        difficulty === "Hard" &&
                                        "normal-phones:size-3 lg-phones:mt-1 sm:size-4 sm:ml-0.5"
                                      }
                                      `}
                              />
                            )}
